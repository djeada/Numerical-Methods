...
These figures can be regenerated from the repository root with:

```bash
python notes/6_regression/resources/plot_gaussian_interpolation.py
```

The plotting script uses a dense evaluation grid that includes every center. Plot sampling controls the smoothness of the drawing; it does not change the mathematical basis functions.

**Terminology:** the [source example](../../src/6_regression/gaussian_interpolation/examples/plot.py) implements Gauss central-difference polynomial interpolation on equally spaced nodes. That is a different method from the Gaussian RBF interpolation described here and generated by the plotting script above.
