...
import pytest
import numpy as np
from ..implementation.backward_difference import (
    backward_difference,
    backward_difference_gradient,
)
...
def test_backward_difference_gradient_mixed():
    ...
    x = np.array([1.0, 2.0, 3.0])
    result = backward_difference_gradient(f, x)
    expected = np.array([x[1], x[0], 3 * x[2] ** 2])
    assert np.allclose(result, expected, rtol=1e-3)
...
def test_backward_difference_gradient_complex_function():
    ...
    x = np.array([1.0, 2.0])
    result = backward_difference_gradient(f, x)
    expected = np.array([x[1] * np.exp(x[0] * x[1]), x[0] * np.exp(x[0] * x[1])])
    assert np.allclose(result, expected, rtol=1e-3)
...
def test_backward_difference_gradient_negative_values():
    ...
    x = np.array([-1.0, -2.0])
    result = backward_difference_gradient(f, x)
    expected = np.array([3 * x[0] ** 2, -2 * x[1]])
    assert np.allclose(result, expected, rtol=1e-3)
...
def test_backward_difference_gradient_high_precision():
    ...
    expected = np.array([4 * x[0] ** 3, 4 * x[1] ** 3])
    assert np.allclose(result, expected, rtol=1e-3)
