...
class TestLinearInterpolation(unittest.TestCase):
    ...
    def test_distance_to_data_points(self):
        ...
        for i, x in enumerate(self.x):
            assert math.isclose(self.y[i], __class__.result_function(x), rel_tol=self.epsilon)

    def test_compare_with_scipy(self):
        cs = CubicSpline(self.x, self.y)
        for i, x in enumerate(self.x):
            assert math.isclose(cs(x), __class__.result_function(x), rel_tol=self.epsilon)
