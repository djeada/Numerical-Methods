...
def newton_polynomial(x_data: np.ndarray, y_data: np.ndarray, point: float) -> float:
    ...
    for j in range(1, n):
        divided_diff[j:n] = (divided_diff[j:n] - divided_diff[j - 1 : n - 1]) / (
            x_data[j:n] - x_data[0 : n - j]
        )
